...
const findOverallGrade = (): number => {
    console.log("\n" + "Enter the marks obtained for each subjects:" + "\n");

    const writtenTest: number = parseFloat(readlineSync.question("Written test = "));
    const labExams: number = parseFloat(readlineSync.question("Lab exams = "));
    const assignments: number = parseFloat(readlineSync.question("Assignments = "));
